...
use crate::{GameStateMemory, InputMemory};
...
pub async fn game(
    ...
    for bars in bars.iter_mut().take(2) {
        for j in 0..(consts::BAR_LAYOUT.len()) {
            bars.push(Bar::new(consts::BAR_LAYOUT[j], consts::BAR_HALF_LEN[j]));
        }
    }
    ...
    loop {
        ...
        if is_on_going {
...

            {
                ...
                for (i, bars) in bars.iter_mut().enumerate().take(2) {
            ...
            for i in 0..2 {
                for unit in consts::BUBBLE_SCHEDULE {
                    if unit.spawn_frame == count {
                        if let Some(bubble_color) = unit.bubble_color {
                            bubbles[i]
                                .push(Bubble::new(consts::BUBBLE_SPAWN_POSITION, bubble_color));
                        } else {
                ...
                for bubble in &mut bubbles[i] {
                    ...
                    if !flag {
                        bubble.move_physically(
                            bubble.position
                                + consts::BUBBLE_GRAVITY
                                + bubble.previous_velocity * consts::INERTIA,
                        );
                    }
                }
            ...
            } = *game_state_memory.lock().await
            {
                for i in 0..2 {
                    ...
                    bubble_positions[i] = bubbles[i].iter().map(|bubble| bubble.position).collect();
                    bubble_colors[i] = bubbles[i].iter().map(|bubble| bubble.color).collect();
                }
            }
        }
